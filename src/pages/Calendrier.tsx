...
const Calendrier = () => {
  ...
  // Mettre à jour automatiquement l'heure de fin lorsque l'heure de début change
  useEffect(() => {
  ...
  // Nouvelle mutation atomique pour décrémenter les heures_restantes
  const decrementHeures = useMutation({
    mutationFn: async () => {
      // Appel d'une fonction RPC côté Postgres pour décrémenter atomiquement
  ...
  const reserverHeure = useMutation({
    mutationFn: async (heureId: string) => {
      console.log('Tentative de réservation pour l\'heure :', heureId, 'par user', user?.id);
      ...
      console.log('Résultat update heures:', { heureError, heureData });

      if (heureError) throw heureError;

      // Décrémenter atomiquement les heures_restantes
      const { data: avant } = await supabase
        .from('utilisateurs')
        .select('heures_restantes')
        .eq('id', user?.id)
        .single();
      console.log('Avant decrement:', avant);

      const { error: decError, data: decData } = await supabase.rpc('decrement_heures', { user_id: user?.id });
      console.log('Résultat decrement_heures:', { decError, decData });
      ...
      const { data: apres } = await supabase
        ...
        .eq('id', user?.id)
        .single();
      console.log('Après decrement:', apres);
      ...
      const { data: userData, error: userFetchError } = await supabase
        ...
        .eq('id', user?.id)
        .single();
      console.log('Nouvelle valeur heures_restantes:', userData, userFetchError);
